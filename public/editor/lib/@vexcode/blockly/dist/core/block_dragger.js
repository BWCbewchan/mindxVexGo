...
goog.require('Blockly.Events.Monitor');
...
Blockly.BlockDragger = function(block, workspace) {
  ...
  /**
   * Which delete area the mouse pointer is over, if any.
   * One of {@link Blockly.MONITOR_AREA_NONE},
   * or {@link Blockly.MONITOR_AREA_DROP}.
   * @type {?number}
   * @private
   */
  this.monitorArea_ = null;
  ...
  // RM Code Start
  /**
   * Whether the block would be monitored if dropped immediately.
   * @type {boolean}
   * @private
   */
  this.wouldMonitorBlock_ = false;

  /**
   * Whether the block drag started from the toolbox.
   * @type {boolean}
   * @private
   */
  this.dragFromToolbox = false;
  // RM Code End
...
Blockly.BlockDragger.prototype.dragBlock = function(e, currentDragDeltaXY) {
  ...
  this.monitorArea_ = this.workspace_.isMonitorArea(e);
  this.draggedConnectionManager_.update(delta, this.deleteArea_, this.monitorArea_);
...
Blockly.BlockDragger.prototype.endBlockDrag = function(e, currentDragDeltaXY) {
  ...
  var delta = this.pixelsToWorkspaceUnits_(currentDragDeltaXY);
  // RM Code Start
  if (this.wouldMonitorBlock_ && !this.dragFromToolbox) {
    delta = new Blockly.utils.Coordinate(0, 0);
  } else if (this.wouldMonitorBlock_ && this.dragFromToolbox) {
    delta.y += 50;
  }
  // RM Code End
  var newLoc = Blockly.utils.Coordinate.sum(this.startXY_, delta);
  ...
  if (!deleted) {
    ...
    if (this.wouldMonitorBlock_) {
      // if this is a monitor move, we know that the last update is no loger correct
      // so we force a new udpate with the 0,0 delta
      this.draggedConnectionManager_.update(delta, this.deleteArea_, this.monitorArea_);
    }
    // RM Code End
  ...
  // RM Code Start
  if (this.wouldMonitorBlock_) {
    if (this.draggingBlock_.monitorSupported) {
      var event = new Blockly.Events.Monitor(this.draggingBlock_, 'monitorBlock', null, null);
      Blockly.Events.fire(event);
    }
    if (this.dragFromToolbox) {
      var workspace = this.draggingBlock_.workspace;
      workspace.undo(false);
      workspace.clearRedoOnly();

      // this.draggingBlock_.dispose(false, true);
    }

    var monitorIcon = this.workspace_.monitorIcon;
    monitorIcon.setState(false, false);
...
Blockly.BlockDragger.prototype.updateCursorDuringBlockDrag_ = function() {
  ...
  // RM Code Start
  this.wouldMonitorBlock_ = this.draggedConnectionManager_.wouldMonitorBlock();
  // RM Code End
  var trashcan = this.workspace_.trashcan;
  if (this.wouldDeleteBlock_) {
  ...
  var monitorIcon = this.workspace_.monitorIcon;
  if (monitorIcon) {
    monitorIcon.setState(this.wouldMonitorBlock_, this.draggingBlock_.monitorSupported);
  }
  // RM Code End
};
