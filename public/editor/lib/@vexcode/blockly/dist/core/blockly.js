 ...
 */
Blockly.duplicate = function(toDuplicate, x, y) {// RM Code Changes
  // Save the clipboard.
  ...
  // RM Codes Start
  var duplicateStack = Blockly.Xml.blockToDom(toDuplicate, true);
  if (!x || !y || x === "" || y === "" || isNaN(x) || isNaN(y)) {
    x = 0;
    y = 0;
  }
  var myDuplicatedBlock = toDuplicate.workspace.RMDuplicate(duplicateStack, x, y);

  // Restore the clipboard.
  Blockly.clipboardXml_ = clipboardXml;
  Blockly.clipboardSource_ = clipboardSource;
  return myDuplicatedBlock;
  // RM Codes End
};
 ...
 */
Blockly.prompt = function(message, defaultValue, callback, opt_title, opt_varType) {
