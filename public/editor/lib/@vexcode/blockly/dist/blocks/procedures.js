...
Blockly.ScratchBlocks.ProcedureUtils.callerMutationToDom = function () {
  ...
  container.setAttribute('proceduredefid', this.procedureDefId);
...
Blockly.ScratchBlocks.ProcedureUtils.callerDomToMutation = function (xmlElement) {
  ...
  this.procedureDefId = xmlElement.getAttribute('proceduredefid');
  const generateshadows = xmlElement.getAttribute('generateshadows');
  this.generateShadows_ = (generateshadows ? JSON.parse(generateshadows) : false) || false;
...
Blockly.ScratchBlocks.ProcedureUtils.definitionMutationToDom = function (
  ...
  container.setAttribute('proccode', this.procCode_);
  container.setAttribute('proceduredefid', this.id);
  container.setAttribute('argumentids', JSON.stringify(this.argumentIds_));
  ...
  container.setAttribute('argumenttypes',
    JSON.stringify(this.argumentTypes_));
  container.setAttribute('warp', JSON.stringify(this.warp_));
  return container;
};
...
Blockly.ScratchBlocks.ProcedureUtils.definitionDomToMutation = function (xmlElement) {
  ...
  this.argumentTypes_ = JSON.parse(
    xmlElement.getAttribute('argumenttypes')) || [];
  this.updateDisplay_();
...
// procedures_call, and procedures_prototype).
/**
 * Returns the uniques function name of the procedure this block calls, or the empty string if
 * it has not yet been set.
 * @return {string} Procedure name.
 * @this Blockly.Block
 */
 Blockly.ScratchBlocks.ProcedureUtils.getFuncName = function() {
  // Split the proc into components, by %n, %b, and %s (ignoring escaped).
  const procComponents = this.procCode_.split(/(?=[^\\]%[nbs])/).map((x) => x.trim());
  ...
  for (var i = 0, component; component = procComponents[i]; i++) {
    ...
    if (component.substring(0, 1) == '%') {
      ...
      funcNameParts.push(this.displayNames_[argumentCount]);
      argumentCount++;
    ...
    if (labelText) {
      funcNameParts.push(labelText.replace(/\\%/, '%'));
    }
  }

  let funcName = funcNameParts.join(" ");
...
Blockly.ScratchBlocks.ProcedureUtils.disconnectOldBlocks_ = function () {
  ...
  for (var i = 0, input; input = this.inputList[i]; i++) {
    if (input.connection) {
      const rawTarget = input.connection.targetBlock();
      const rawShadow = input.connection.getShadowDom();
      
      const target = rawTarget.isShadow() ? null : rawTarget;
      const shadow = rawTarget.isShadow() && !rawShadow ? Blockly.Xml.blockToDom(rawTarget, true) : rawShadow;
      var saveInfo = {
        shadow,
        block: target
      };
      ...
      if (target && input.connection.isConnected()) {
...
Blockly.ScratchBlocks.ProcedureUtils.buildShadowDom_ = function (type) {
  ...
  if (type == 'n') {
    ...
    var shadowType = 'procedure_bool_input';
    var fieldName = 'BOOLVALUE';
    var fieldValue = 'true';
  } else {
    var shadowType = 'text';
    var fieldName = 'TEXT';
...
Blockly.ScratchBlocks.ProcedureUtils.attachShadow_ = function (input,
    argumentType) {
  const argToBlockType = () => {
    switch (argumentType) {
      case "n": return "math_number";
      case "b": return "procedure_bool_input";
      case "s": return "text"
      default:  return null;
    }
  }
  var blockType = argToBlockType();
  if (blockType) {
    ...
    try {
      ...
      if (argumentType == 'n') {
        ...
        newBlock.setFieldValue('true', 'BOOLVALUE');
      } else {
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentReporter_ = function (
  ...
  try {
    var newBlock = this.workspace.newBlock(blockType);
    newBlock.funcName = this.getFuncName();
    if (!newBlock.procedureDefId && this.id) {
      newBlock.procedureDefId = this.id;
    }
    newBlock.setShadow(true);
...
Blockly.ScratchBlocks.ProcedureUtils.populateArgumentOnPrototype_ = function (
  ...
  // RM code start
  argumentReporter.argumentId = this.argumentIds_[index];
  // RM code end
 ...
 * @param {string} type The argument type.  One of 'b', 'n', or 's'.
  ...
  for (var i = 0; i < this.inputList.length; i++) {
    ...
    if (input.type == Blockly.DUMMY_INPUT) {
      ...
      if (target.type == 'argument_editor_boolean') {
        this.procCode_ += '%b';
        this.argumentTypes_.push("b");
      } else if (target.type == 'argument_editor_number') {
        this.procCode_ += '%n';
        this.argumentTypes_.push("n");
      } else {
        this.procCode_ += '%s';
        this.argumentTypes_.push("s");
      }
    } else {
      throw new Error(
...
Blockly.ScratchBlocks.ProcedureUtils.focusLastEditor_ = function () {
  if (this.inputList.length > 0) {
    ...
    if (newInput.type == Blockly.DUMMY_INPUT) {
      // use timeout to make sure the editor opens after the block shifts
      setTimeout(() => {
...
Blockly.Blocks['procedures_call'] = {
  ...
  getFuncName: Blockly.ScratchBlocks.ProcedureUtils.getFuncName,
...
Blockly.Blocks['argument_reporter_boolean'] = {
  init: function () {
    this.jsonInit({
      ...
      "mutator": "data_set_scope_mutator",
    });
  }
};
 ...
 * @package
 */
Blockly.ScratchBlocks.ProcedureUtils.SET_SCOPE_MUTATOR_MIXIN = {
  
  /**
   * generate mutation dom for given funcName
   * @param {String} newFuncName the new function name
   * @return {Element} XML storage element.
   */
  generateMutation: function(newFuncName) {
    var container = document.createElement('mutation');
    container.setAttribute('funcname', newFuncName ? newFuncName : "");
    container.setAttribute('argumentid', this.argumentId ? this.argumentId : "");
    container.setAttribute('proceduredefid', this.procedureDefId ? this.procedureDefId : "");
  ...
  mutationToDom: function() {
    return this.generateMutation(this.funcName);
  },

  /**
   * Parse XML to restore the 'funcName'.
   * @param {!Element} xmlElement XML storage element.
   * @this Blockly.Block
   */
  domToMutation: function(xmlElement) {
    this.funcName = xmlElement.getAttribute('funcname');
    this.argumentId = xmlElement.getAttribute('argumentid');
    this.procedureDefId = xmlElement.getAttribute('proceduredefid');
    // console.log("this.procedureDefId:", this.procedureDefId);
  },
...
/**
 * 'data_set_scope_mutator' extension to the argument_report_* blocks.
 * @this Blockly.Block
 * @package
 */
Blockly.ScratchBlocks.ProcedureUtils.SET_SCOPE_MUTATOR_EXTENSION = function() {
...
Blockly.Extensions.registerMutator(
  'data_set_scope_mutator',
  Blockly.ScratchBlocks.ProcedureUtils.SET_SCOPE_MUTATOR_MIXIN,
  Blockly.ScratchBlocks.ProcedureUtils.SET_SCOPE_MUTATOR_EXTENSION
);
