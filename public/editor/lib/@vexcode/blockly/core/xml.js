...
Blockly.Xml.workspaceToDom = function(workspace, opt_noId) {
  ...
  // RM Code Start
  var variablesElement = Blockly.Xml.variablesToDom(workspace.getAllVariables());
  // RM Code End
...
Blockly.Xml.variablesToDom = function(variableList) {
  ...
  for (var i = 0, variable; (variable = variableList[i]); i++) {
    ...
    if (variable.arrayLength) {
      element.setAttribute('arraylength', variable.arrayLength);
    }
    if (variable.arrayWidth) {
      element.setAttribute('arraywidth', variable.arrayWidth);
    }
...
Blockly.Xml.domToVariables = function(xmlVariables, workspace) {
  for (var i = 0, xmlChild; (xmlChild = xmlVariables.childNodes[i]); i++) {
    ...
    var arrayLength = parseInt(xmlChild.getAttribute('arraylength') || 0);
    var arrayWidth = parseInt(xmlChild.getAttribute('arraywidth') || 0);
    var name = xmlChild.textContent;

    workspace.createVariable(name, type, id, arrayLength, arrayWidth);
  }
};
...
Blockly.Xml.domToBlockHeadless_ = function(xmlBlock, workspace) {
  ...
  if (xmlBlock.nodeName.toLowerCase() == 'shadow') {
    ...
    for (var i = 0, child; (child = children[i]); i++) {
      if (!child.isShadow() && block.type !== "data_2darrayrow") {
        // RM Code Start
        // Added '&& block.type !== "data_2darrayrow"' to the condition above for allowing
        // Noraml blocks in shadow blocks in 2d array: https://robomatter.atlassian.net/browse/VC-8598
        // RM Code End
        throw TypeError('Shadow block not allowed non-shadow child.');
      } 
    }
    // RM Code Start
    // We need variables in shadow blocks for broadcast blocks...
