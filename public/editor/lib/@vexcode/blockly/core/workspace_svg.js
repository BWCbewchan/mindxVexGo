...
Blockly.WorkspaceSvg = function(options,
  ...
  */
  ...
  // RM - add typed variables to the flyout lists
  if (Blockly.VariablesTyped && Blockly.VariablesTyped.flyoutCategory) {
    this.registerToolboxCategoryCallback(Blockly.VARIABLE_TYPED_CATEGORY_NAME,
        Blockly.VariablesTyped.flyoutCategory);
  }
  ...
  if (options.zoomOptions && options.zoomOptions.startScale) {
    this.scaleStart = Number(options.zoomOptions.startScale);
  } else {
 ...
 * @type {number}
 */
 Blockly.WorkspaceSvg.prototype.scaleStart = 1;
...
// RM Code Start
/**
 * The workspace's monitor icon (if any).
 * @type {Blockly.MonitorIcon}
 */
Blockly.WorkspaceSvg.prototype.monitorIcon = null;
// RM Code End
...
Blockly.WorkspaceSvg.prototype.dispose = function() {
  ...
  // RM Code Start
  if (this.monitorIcon) {
    this.monitorIcon.dispose();
    this.monitorIcon = null;
  }
  // RM Code End
 ...
 * @package
 */
 Blockly.WorkspaceSvg.prototype.addMonitorIcon = function() {
  if (!Blockly.MonitorIcon) {
    throw Error('Missing require for Blockly.MonitorIcon');
  }
  /** @type {Blockly.MonitorIcon} */
  this.monitorIcon = new Blockly.MonitorIcon(this);
  var svgMonitorIcon = this.monitorIcon.createDom();
  this.svgGroup_.insertBefore(svgMonitorIcon, this.svgBlockCanvas_);
};
// RM Code End
 ...
 *     DocumentFragment if the block was an insertion marker.
 */
Blockly.WorkspaceSvg.prototype.RMDuplicate = function(xmlBlock, x, y) {
  try {
    var block = Blockly.Xml.domToBlock(xmlBlock, this);
    if(!Number.isNaN(x) && !Number.isNaN(y)) {
      block.moveBy(x, y);
    }
    // Blockly.navigation.insertBlock(/** @type {!Blockly.BlockSvg} */ (block), block.nextConnection.type);
    return block;
  } catch (error) {
...
Blockly.WorkspaceSvg.prototype.createVariable = function(name,
    opt_type, opt_id, opt_arrayLength, opt_arrayWidth) {
  var newVar = Blockly.WorkspaceSvg.superClass_.createVariable.call(
      this, name, opt_type, opt_id, opt_arrayLength, opt_arrayWidth);
  this.refreshToolboxSelection();
  return newVar;
};
...
Blockly.WorkspaceSvg.prototype.recordDeleteAreas = function() {
  ...
  // RM Code Start
  if (this.monitorIcon && this.svgGroup_.parentNode) {
    this.monitorAreaIcon_ = this.monitorIcon.getClientRect();
  } else {
    this.monitorAreaIcon_ = null;
  }
  this.deleteAreaFlyout_ = null;
  // RM Code End
  if (this.flyout_) {
    ...
    // RM Code Start
    if (this.toolbox_.flyout_ && this.toolbox_.flyout_.isVisible()) {
      this.deleteAreaFlyout_ = this.toolbox_.flyout_.getClientRect();
    }
    // RM Code End
  } else {
...
Blockly.WorkspaceSvg.prototype.isDeleteArea = function(e) {
  ...
  // RM Code Start
  if (this.deleteAreaFlyout_ && this.toolbox_.flyout_ && this.toolbox_.flyout_.isVisible() &&
      this.deleteAreaFlyout_.contains(e.clientX, e.clientY)) {
    return Blockly.DELETE_AREA_TOOLBOX;
  }
  // RM Code End
 ...
 *     which delete area the event is over.
 */
 Blockly.WorkspaceSvg.prototype.isMonitorArea = function(e) {
  if (this.monitorAreaIcon_ &&
      this.monitorAreaIcon_.contains(e.clientX, e.clientY)) {
    return Blockly.MONITOR_AREA_DROP;
  }
  return Blockly.MONITOR_AREA_NONE;
};
// RM Code End
...
Blockly.WorkspaceSvg.prototype.showContextMenu = function(e) {
  ...
  var menuOptions = Blockly.ContextMenuRegistry.registry.getContextMenuOptions(
      Blockly.ContextMenuRegistry.ScopeType.WORKSPACE, {workspace: this, event: e});
 ...
 * @public
 */
Blockly.WorkspaceSvg.prototype.moveWorkspaceTopLeft = function(id) {
  if (!this.isMovable()) {
    console.warn('Tried to move a non-movable workspace. This could result' +
        ' in blocks becoming inaccessible.');
    return;
  ...
  var xy = block.getRelativeToSurfaceXY();
  // Height/width is in workspace units.
  // The centerOnBlock function grabs the height of the whole stack, placing the stack in the center instead of the passed in block
  // This function references just the block instead
  ...
  // viewHeight and viewWidth are in pixels.
  var halfViewWidth = metrics.viewWidth;
  var halfViewHeight = metrics.viewHeight;

  // In the centerOnBlock function, this was set to 2 instead of 6 to put it in the center, 
  // now we are putting it in the top left by dividing by 6
  var scrollToCenterX = pixelX - halfViewWidth / 6;
  var scrollToCenterY = pixelY - halfViewHeight / 6;
...
Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function() {
  ...
  if (this.toolbox_) {
    if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP ||
      ...
      // RM Code Start
      // we need to consider the toolbox flyout as well when it is open
      if (this.toolbox_ && this.toolbox_.flyout_ && this.toolbox_.flyout_.isVisible()) {
        // Only overwrite the supposed toolbox height when the flyout is open
        viewSize.height -= this.toolbox_.flyout_.getHeight();
      }
      // RM Code End
      ...
      if (this.toolbox_ && this.toolbox_.flyout_ && this.toolbox_.flyout_.isVisible()) {
        // Only overwrite the supposed toolbox height when the flyout is open
        viewSize.width -= this.toolbox_.flyout_.getWidth();
      }
      // RM Code End
    }
  ...
  if (this.toolbox_ && this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT) {
    ...
    if (this.toolbox_ && this.toolbox_.flyout_ && this.toolbox_.flyout_.isVisible()) {
      // Only overwrite the supposed toolbox height when the flyout is open
      absoluteLeft += this.toolbox_.flyout_.getWidth();
    }
    // RM Code End
 ...
 */
Blockly.WorkspaceSvg.prototype.refreshToolbox = function() {
  const toolbox = this.getToolbox();
  const flyout = toolbox.getFlyout();
  const hideAfterRefresh = flyout.autoClose && !flyout.isVisible();
  this.refreshToolboxSelection();
  if (hideAfterRefresh) {
